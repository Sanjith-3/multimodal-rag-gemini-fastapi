...
class RAGEvaluator:
    ...
    def calculate_text_metrics(self, generated_response, reference_response):
        # BLEU Score
        reference = [reference_response.split()]
        ...
        # Semantic Similarity
        gen_embedding = self.text_embedding_model.encode([generated_response])
        ref_embedding = self.text_embedding_model.encode([reference_response])
    ...
    def evaluate_image_similarity(self, reference_image, retrieved_image):
        # Multimodal LLM-based image similarity
...
async def evaluate(
    ...
    # Text Metrics
    text_metrics = evaluator.calculate_text_metrics(
        answer, 
        reference_response
    )
    
    # Convert numpy types to Python native types
    text_metrics_converted = {
    ...
    image_metrics = {}
    try:
        if mm_rag.last_query_context and mm_rag.last_query_context.get('retrieved_images'):
            retrieved_images = mm_rag.last_query_context['retrieved_images']
            
            image_similarities = []
            for ref_img, ret_img in zip(base64_images, retrieved_images):
                similarity = evaluator.evaluate_image_similarity(ref_img, ret_img)
                if similarity is not None:
                    image_similarities.append(float(similarity))
            
            if image_similarities:
                image_metrics['avg_image_similarity'] = sum(image_similarities) / len(image_similarities)
    except Exception as e:
        logging.error(f"Image metrics calculation error: {e}")
    
    return {
        "answer": answer,
        "reference_context": base64_images,
        "reference_response": reference_response,
        "text_metrics": text_metrics_converted,
