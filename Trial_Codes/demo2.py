...
class DeepEvalMetricsHandler:
    def __init__(self, custom_gemini):
        # Wrap the LangChain model in our custom wrapper
        wrapped_model = CustomModelWrapper(custom_gemini)
    ...
    def evaluate_response(self, 
                         ...
                         actual_output: str,
                         expected_output: str,
                         retrieval_context: List[str]) -> dict:
        """
        ...
        for metric in self.metrics:
            try:
                ...
                logging.error(f"Error measuring {metric.__class__.__name__}: {str(e)}")
                results[metric.__class__.__name__] = {
                    "score": None,
...
from deepeval.test_case import LLMTestCase
from deepeval import LLMTestCase, BaseModel
from langchain_google_genai import ChatGoogleGenerativeAI
...
class CustomModelWrapper(BaseModel):
    ...
    def generate(self, prompt: str) -> str:
        """Convert the LangChain model's generation interface to DeepEval's expected interface"""
        try:
            # Convert LangChain model output to string
...
custom_gemini = ChatGoogleGenerativeAI(
    model="gemini-pro",
    temperature=0,
    max_tokens=1024,
    top_p=0.95,
...
@app.post("/evaluate")
async def evaluate(
    input_query: str = Form(...),  # Added input_query as required parameter
    ...
    regular_evaluator = RAGEvaluator()
    deep_eval_handler = DeepEvalMetricsHandler(custom_gemini)
    ...
    # Process uploaded images (existing logic)
    ...
    existing_evaluation = regular_evaluator.evaluate_all(
    ...
    # Get DeepEval metrics using the provided input_query
    try:
        deep_eval_results = deep_eval_handler.evaluate_response(
        ...
        logging.error(f"Error in DeepEval evaluation: {str(e)}")
        deep_eval_results = {"error": str(e)}
    ...
    combined_results = {
        ...
        "reference_response": reference_response,
        "reference_context": reference_context,
        "traditional_metrics": convert_numpy(existing_evaluation),
        "deep_eval_metrics": deep_eval_results
    }
